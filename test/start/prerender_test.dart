...
void main() {
  test('prerenderer fetches the real app and crawls local links', () async {
    ...
    final root = AppRoute<NoParams, NoSearch, NoData>(
      ...
      children: <AnyAppRoute>[
        AppRoute<NoParams, NoSearch, NoData>(
          path: 'posts',
          terminal: false,
          children: <AnyAppRoute>[post],
        ),
      ],
    );
    final server = await StartIoServer.bind(
      StartApplication(routes: <AnyAppRoute>[root]).handler,
      port: 0,
    );
    ...
    final prerenderer = StartPrerenderer();
    final result = await prerenderer.render(
      origin: Uri.parse('http://127.0.0.1:${server.port}'),
      routes: const <String>['/'],
      output: output,
      options: const StartPrerenderOptions(concurrency: 2),
    );

    expect(result.routes.map((route) => route.route), <String>[
      '/',
      '/posts/42',
    ]);
    expect(
    ...
    expect(repeated.routes.single.route, '/posts/42');
  });
}
