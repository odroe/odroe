...
final class FileRouteCompiler {
  ...
  String _generateServer(_RouteNode root, List<_RouteNode> nodes) {
    ...
    for (final node in nodes) {
      for (final function in node.functions) {
        ...
        final decoder = _wireDecoder(input, 'value');
        buffer.writeln(
          '  ${jsonEncode(id)}: ${node.serverAlias}.${function.name}.bind('
          '${jsonEncode(id)},'
  ...
  void _writeFacade(
    ...
    for (final function in node.functions) {
      ...
      final decoder = _wireDecoder(output, 'value');
      buffer
        ..writeln()
        ..writeln(
...
String? _validateWireType(
  ...
  if (base == 'StartResponse') {
    return !input && topLevel
        ? null
        : 'StartResponse is only valid as direct output';
  }
  if (const <String>{
    'ByteBuffer',
    'ByteData',
    ...
    return '$base has no built-in Start serializer';
  }
  if (base == 'Map') {
...
String? _wireDecoder(String source, String value) {
