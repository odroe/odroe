...
- [TanStack Router/Start](https://github.com/TanStack/router/tree/31a634d84ecf393dfb95adcf713fd5f1a13ab347)：route match 的 head 合并、SSR hydration、prerender queue、静态路由发现与真实 preview server 请求；
...
只有 Document 时，它就是纯 HTML route。两者同时存在时，Start 首次请求输出 Document、loader/Query handoff 与 Flutter bootstrap；Flutter 第一帧后隐藏辅助 Document DOM，后续 Flutter 路由继续复用首屏数据。是否加载 Flutter 由当前匹配终点决定，不因应用内其他 route 存在 `page.dart` 而污染纯 HTML route。Flutter 原生目标只使用 page，不需要 DOM。
...
- 文本和属性默认转义；tag 与 attribute name 必须合法；void element 禁止 children；
...
- SSG 不直接调用另一套 renderer，而是请求构建后的真实 Start HTTP server；
- 预渲染只写站内、无 query、无 traversal 的 HTML route，并限制路径长度；
- 静态 route 自动枚举，动态 route 由真实 HTML 链接发现；并发、重试和 fail-on-error 是构建行为，不是用户维护的清单或 hash 合同。

## CLI 决策

- `odroe dev -- -d chrome`：浏览器打开 Start origin；Start 提供 SSR HTML，并同源代理 Flutter DevFS、SSE 调试与资源；
- 其他 Flutter target：Start 与 `flutter run` 并行，target 仍完全交给 Flutter CLI；
- `odroe build -- web --release`：生成 routes，构建 Start、构建 Flutter Web，再请求 Start 完成 SSG；
